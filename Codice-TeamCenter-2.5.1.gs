...
function normalizzaChiaveSquadra_(idSquadra) {
  return String(idSquadra || "")
    .trim()
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, "_")
    .replace(/^_+|_+$/g, "");
...
function chiavePasswordSquadra_(idSquadra) {
...
function loginSquadraTeamCenter_(parametri) {
  ...
  if (!idSquadra) throw new Error("Gruppo squadra obbligatorio.");
  verificaSquadraEsistente_(idSquadra);

  const chiave = chiavePasswordSquadra_(idSquadra);
  const proprieta = PropertiesService.getScriptProperties();
  let passwordCorretta = proprieta.getProperty(chiave);
  ...
  if (!passwordInserita || passwordInserita !== passwordCorretta) {
    throw new Error("Password del gruppo squadra non corretta.");
  }

  const token = Utilities.getUuid() + Utilities.getUuid();
  CacheService
    .getScriptCache()
    .put("TEAM_SESSION_" + token, idSquadra, 3600);
...
function verificaSessioneSquadraTeamCenter_(token) {
  const tokenPulito = String(token || "").trim();
  if (!tokenPulito) throw new Error("Sessione squadra mancante.");

  const idSquadra = CacheService
    .getScriptCache()
    .get("TEAM_SESSION_" + tokenPulito);

  if (!idSquadra) {
    throw new Error("Sessione squadra scaduta. Accedi nuovamente.");
  }

  return { valida: true, idSquadra: idSquadra };
}

function logoutSquadraTeamCenter_(token) {
  const tokenPulito = String(token || "").trim();
  if (tokenPulito) {
    CacheService
      .getScriptCache()
      .remove("TEAM_SESSION_" + tokenPulito);
  }
  return { logout: true };
}

function assicuraGruppiScuolaCalcio_() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const foglio = spreadsheet.getSheetByName("SQUADRE");
  if (!foglio) throw new Error("Foglio SQUADRE non trovato.");
  ...
  const indiceId = intestazioni.indexOf("IDSquadra");
  const indiceNome = intestazioni.indexOf("NomeSquadra");
  const indiceAttiva = intestazioni.indexOf("Attiva");
  if (indiceId < 0 || indiceNome < 0) return;

  const gruppi = [
    { IDSquadra: "SC-ESORDIENTI-2014", NomeSquadra: "ESORDIENTI 2014" },
    { IDSquadra: "SC-ESORDIENTI-2015", NomeSquadra: "ESORDIENTI 2015" },
    { IDSquadra: "SC-PULCINI-2016", NomeSquadra: "PULCINI 2016" },
    { IDSquadra: "SC-PULCINI-2017", NomeSquadra: "PULCINI 2017" },
    { IDSquadra: "SC-PRIMI-CALCI-2018", NomeSquadra: "PRIMI CALCI 2018" },
    { IDSquadra: "SC-PRIMI-CALCI-2019", NomeSquadra: "PRIMI CALCI 2019" },
    { IDSquadra: "SC-PICCOLI-AMICI-2020-2021", NomeSquadra: "PICCOLI AMICI 2020-2021" }
  ];

  const vecchiGruppi = new Set([
    "SC-ESORDIENTI",
    "SC-PULCINI",
    "SC-PRIMI-CALCI",
    "SC-PICCOLI-AMICI"
  ...
  const righe = lastRow > 1
    ? foglio.getRange(2, 1, lastRow - 1, ultimaColonna).getDisplayValues()
    : [];

  const esistenti = new Set();
  righe.forEach((riga, index) => {
    const id = String(riga[indiceId] || "").trim().toUpperCase();
    if (id) esistenti.add(id);
    if (vecchiGruppi.has(id) && indiceAttiva >= 0) {
      foglio.getRange(index + 2, indiceAttiva + 1).setValue("NO");
    }
  });

  gruppi.forEach(gruppo => {
    if (esistenti.has(gruppo.IDSquadra)) {
      // Se il nuovo gruppo esiste già, lo riattiviamo.
      righe.forEach((riga, index) => {
        const id = String(riga[indiceId] || "").trim().toUpperCase();
        if (id === gruppo.IDSquadra && indiceAttiva >= 0) {
          foglio.getRange(index + 2, indiceAttiva + 1).setValue("SI");
        }
      });
      return;
    }
    ...
    const valori = intestazioni.map(campo => {
      if (campo === "IDSquadra") return gruppo.IDSquadra;
      if (campo === "NomeSquadra") return gruppo.NomeSquadra;
      if (campo === "Attiva") return "SI";
      if (campo === "MacroGruppo") return "SCUOLA CALCIO";
      return "";
    });
    foglio.appendRow(valori);
  });
...
function doGet(e) {
  try {
    ...
    switch (action) {
      ...
      case "loginsquadra":
        dati = loginSquadraTeamCenter_(parametri);
        break;

      case "verificasessionesquadra":
        dati = verificaSessioneSquadraTeamCenter_(parametri.token);
        break;

      case "logoutsquadra":
        dati = logoutSquadraTeamCenter_(parametri.token);
        break;
