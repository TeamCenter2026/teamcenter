...
function generaReportAllenamentiTeamCenter_(parametri) {
  ...
  allenamenti.forEach(allenamento => {
    ...
    presenze.forEach(item => {
      ...
      dettaglio.push([
        dataItalianaReport_(allenamento.DataReport),
    ...
    cronologia.push([
      ...
      conteggi.Presente,
      conteggi.Assente,
      conteggi.Giustificato,
      conteggi.Infortunato,
      presenze.length
    ]);
  });
  ...
  const righeStatistiche = Array.from(statistiche.values())
    .map(voce => {
      const percentuale = voce.SeduteRegistrate
        ? Math.round((voce.Presente / voce.SeduteRegistrate) * 10000) / 100
        : 0;
      return [
        voce.IDGiocatore,
        voce.Cognome,
    ...
    .sort((a, b) => b[9] - a[9] || b[5] - a[5] || String(a[1]).localeCompare(String(b[1]), "it"));

  const migliore = righeStatistiche[0] || null;
  const giocatorePiuPresente = migliore ? `${migliore[1]} ${migliore[2]}`.trim() : "";
  const percentualeMigliore = migliore ? `${Math.round(migliore[9] * 10000) / 100}%` : "";
  ...
  const riepilogo = {
    ...
    giocatorePiuPresente: giocatorePiuPresente,
    percentualeMigliore: percentualeMigliore
  };

  const nomeFileBase = `Report_Allenamenti_${idSquadra}_${dal}_${al}`;
  const temporaneo = SpreadsheetApp.create(nomeFileBase);
  const fileId = temporaneo.getId();

  try {
    const riepilogoFoglio = temporaneo.getSheets()[0];
    riepilogoFoglio.setName("Riepilogo");
    preparaFoglioRiepilogo_(riepilogoFoglio, {
    ...
    const dettaglioFoglio = temporaneo.insertSheet("Dettaglio presenze");
    scriviTabellaReport_(dettaglioFoglio,
      ["Data", "ID Allenamento", "ID Giocatore", "Cognome", "Nome", "Anno", "Stato"],
      dettaglio
    );

    const statisticheFoglio = temporaneo.insertSheet("Statistiche giocatori");
    scriviTabellaReport_(statisticheFoglio,
      ["ID Giocatore", "Cognome", "Nome", "Anno", "Sedute registrate", "Presenze", "Assenze", "Giustificate", "Infortuni", "% presenza"],
      righeStatistiche
    );
    if (righeStatistiche.length) {
      statisticheFoglio.getRange(2, 10, righeStatistiche.length, 1).setNumberFormat("0.00%");
    }

    const cronologiaFoglio = temporaneo.insertSheet("Cronologia");
    scriviTabellaReport_(cronologiaFoglio,
      ["Data", "ID Allenamento", "Presenti", "Assenti", "Giustificati", "Infortunati", "Totale giocatori"],
      cronologia
    );
    ...
    if (risposta.getResponseCode() !== 200) {
      throw new Error(`Esportazione Excel non riuscita (${risposta.getResponseCode()}).`);
    }

    const blob = risposta.getBlob().setName(`${nomeFileBase}.xlsx`);
    return {
      nomeFile: blob.getName(),
    ...
    try {
      ...
      console.warn("Impossibile spostare nel cestino il file temporaneo del report.");
    }
  }
}
