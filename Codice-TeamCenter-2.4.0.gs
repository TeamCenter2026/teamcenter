...
function assicuraGruppiScuolaCalcio_() {
  ...
  if (!intestazioni.includes("IDSquadra") || !intestazioni.includes("NomeSquadra")) {
    return;
  }
  ...
  const esistenti = new Set(
    leggiFoglioComeOggetti_("SQUADRE")
      .map(item => String(item.IDSquadra || "").trim().toUpperCase())
      .filter(Boolean)
  );

  gruppi.forEach(gruppo => {
    if (esistenti.has(gruppo.IDSquadra)) return;
